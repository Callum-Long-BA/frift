...
- **Shared date axis per section.** Every Weight training chart uses the same date range and labels, and so does every Cardio & calisthenics chart, so charts side by side line up. The range runs from the first date anything in that section was logged (or the start of the 8 weeks / chosen range, if later) to today. Zooming with the slider in the expanded view is the exception: the axis follows what you select.
- **Breaks in training.** If someone goes more than 8 weeks (56 days) between sessions of an exercise, their line stops and starts again, rather than drawing one long line across the break.
...
| `POST /api/exercises` | Add an exercise. Body: `{ "name": "Romanian deadlift", "personId": 1, "kind": "strength", "equipmentChoice": true }`. `kind` is `strength` (weight × reps, the default) or `reps` (reps only; `equipmentChoice` is ignored). Fails with `409` if the name exists or 30 exercises exist. |
...
- **Limits:** 10 people, 30 exercises, 10 sets per save, weights in kg.
